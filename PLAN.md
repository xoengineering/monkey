...
- **iCloud Drive (opt-in).** `FileManager.url(forUbiquityContainerIdentifier:)` + `Documents/Conversations/`. Requires the iCloud entitlement (CloudDocuments service, one container ID) and an iCloud-capable provisioning profile. Does not require the network entitlement — the iCloud daemon moves bytes, not the app. **[verified]** via throwaway spike `spike/2-icloud-documents-sync` (not merged): a sandboxed macOS app with only `com.apple.developer.icloud-services=[CloudDocuments]` + `com.apple.developer.icloud-container-identifiers=[iCloud.<bundle-id>]` — no network entitlement, no `aps-environment` — resolved its ubiquity container and wrote a file that `bird`/CloudDocs picked up for sync, confirmed via `brctl log`. Cross-device arrival not yet confirmed (no second Mac available to test with; an iPhone/iPad can't see the container without Files-app document-browsing support, which is out of scope for v1 per below).
