...
**Remaining Phase 6 gaps, deliberately deferred rather than half-built:** per-conversation migration progress UI (there's a global spinner, not one row per folder) and "sending disabled during migration" (not wired to the composer yet); `NSMetadataQuery` download-on-demand + placeholder rows; `NSFileVersion` conflict resolution for `conversation.yaml`; the iCloud entitlements themselves (`com.apple.developer.icloud-services`, `com.apple.developer.icloud-container-identifiers`) still need adding to `Monkey.entitlements` before iCloud mode can do anything beyond the verified graceful-failure path above; and the two-device/iCloud-signed-out test matrix from §7's Phase 6 line — none of that is possible to execute solo, overnight, without a second physical device and a real iCloud account signed into this one.
