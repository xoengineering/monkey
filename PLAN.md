...
This isolates the cause precisely: it is not the CLI, not the app-group entitlement, not `Monkey`-specific code — it's *any* `com.apple.security.app-sandbox` binary launched via direct `fork`/`exec` from this specific agent shell, versus the same binary launched via `open`/LaunchServices (which works). The most likely explanation is that this coding agent's own shell process runs under some confinement imposed by the harness it executes in, and a child's app-sandbox initialization can't complete cleanly under that nesting — not a macOS 27 defect, and not evidence that a real user's interactive Terminal.app session would hit the same wall. **This needs verification outside the agent environment**: run `Monkey.app/Contents/Helpers/monkey` from an actual Terminal.app window on a real, interactively-logged-in Mac before trusting either outcome. Until then, treat "does the CLI actually run from Terminal" as unverified — build and test the CLI's *logic* via `swift test` (no sandboxed exec required), but don't assume the packaging works end to end.
