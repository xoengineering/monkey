...
- Markdown rendering: Textual `StructuredText` with the `.gitHub` style preset, text selection enabled (`.textual.structuredTextStyle(.gitHub)`, `.textual.textSelection(.enabled)`). **Important finding**: Textual's *default* image attachment loader (`.image()`, used automatically if you don't override it) fetches remote images over the network via `URLSession`-backed `ImageLoader.shared` — a silent, hard violation of this app's no-network rule the moment a message body contains a markdown image link. Fixed by supplying a custom `NoFetchImageAttachmentLoader` (registered once at the app root via `.textual.imageAttachmentLoader(...)`) that never performs I/O and renders a tappable `Link` showing the URL instead — this is both the network-safety fix and the "remote images as placeholder" UI behavior in one. Anyone touching Textual integration later must not remove this override. Copy button on code blocks: implemented as `CopyableCodeBlockStyle` (`Sources/MonkeyUI/CopyableCodeBlockStyle.swift`) — wraps `StructuredText.GitHubCodeBlockStyle`'s rendering and overlays a button calling `CodeBlockProxy.copyToPasteboard()` (a public Textual API built for exactly this). Code compiles and the underlying code block itself renders correctly with real syntax highlighting (confirmed live). **Unverified**: whether the overlaid button actually appears/is tappable — a live screenshot showed the code block rendering correctly but no visible button in the corner, and a follow-up test with an intentionally oversized, high-contrast debug button (to confirm whether `.overlay` renders at all in this position) was in progress when the session ended. `StructuredText.CodeBlock` is a genuine SwiftUI `View` (confirmed by reading Textual's source — not a TextKit attachment), so there's no obvious structural reason an overlay wouldn't work; needs a fresh look before trusting this feature. Footnotes not supported by Textual's default parser; accepted for v1.
