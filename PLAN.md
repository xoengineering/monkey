...
**Implemented and [verified] live, end to end, including a real bug caught by testing rather than assuming.** `CLIInstaller.swift` (macOS-only, isolated `AppKit` import) presents the panel and creates the symlink via `FileManager.createSymbolicLink`; wired into a new "Command Line Tool" section in `SettingsView`. First attempt failed for a real reason: clicking the button produced no visible panel and no symlink — `log show --predicate 'process == "Monkey"'` revealed why: `Unable to display save panel: your app is missing the User Selected File Read/Write app sandbox entitlement.` This was a wrong assumption on my part (that `NSSavePanel` "just works" under App Sandbox with no extra entitlement) caught by actually testing it rather than shipping it unverified. Fixed by adding `com.apple.security.files.user-selected.read-write` to `Monkey.entitlements` — a plain boolean entitlement with no App ID capability or portal-side resource to create, so (unlike the iCloud container above) `-allowProvisioningUpdates` handled it with no manual Xcode step, and the build stayed green. Retested after the fix: the panel appeared, defaulted into `bin` (i.e. `/usr/local/bin`), and clicking "Install" created a real working symlink (`/usr/local/bin/monkey -> .../Monkey.app/Contents/Helpers/monkey`), confirmed via `ls -la`. Removed the test symlink afterward — this was verification, not a feature the user asked to actually have installed on their machine.
