...
Not implemented this session: the "Install command line tool…" `NSSavePanel` flow in Settings (§6 UI work, not CLI logic) and `monkey see` (already marked v2 above).
...
5. **CLI** — `monkey` target, entitlements, `ls`/`do`/`chat`/`cat`/`open`/`search`, install flow. Test that a Terminal-launched sandboxed binary reads the group container and reaches the model. Done: all six commands implemented and built against `MonkeyCore`, verified end to end against the real on-device model via the unsigned `swift build` product (see §5 for exactly what was and wasn't exercised this way). Not done: the "Install command line tool…" Settings flow, and testing the actual sandboxed `Monkey.app/Contents/Helpers/monkey` binary launched directly from Terminal — blocked on the App-Sandbox-Terminal-exec platform bug (§5), unresolved and not retested this session. Commit.
